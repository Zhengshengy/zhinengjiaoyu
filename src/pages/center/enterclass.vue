                    ...
                    <Split v-model="split4" mode="vertical">
                        <div slot="top" class="demo-split-pane" style="height: 100%;">
                            <div style="height: 15%;border-bottom: 1px solid #cccccc;text-align: center">
                                <div class="classText" :class="{'actives':indText === 1}" @click="startRTC">{{$t('classroom.camera')}}</div>
                            </div>
                            <div style="height: 85%;" v-if="classification==1">
                                <div class="cutindex" v-if="indcut==0">
                                    <div :class="me_video">
                                        <video id="local_video" class="barderVideo" muted="true" autoplay playsinline></video>
                                    </div>
                                    <div :class="remote_video" v-for="(value, key, index) in remoteVideos" @click="conversion(0)">
                                        <div class="col-md-1">
                                            <video :id="key" autoplay playsinline style="width:100%"></video>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="row" style="position: absolute;left: 20px;bottom: 0;cursor: pointer" v-if="indcut==0||indcut==1">
                                <Icon type="md-eye" size="24" @click="toggleCamera" v-if="enableCamera==true"/>
                                <Icon type="md-eye-off" size="24" @click="toggleCamera"
                                      v-else-if="enableCamera==false"/>
                                <Icon type="ios-mic" size="24" @click="toggleMic" v-if="enableMic==true"/>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 创建或者进入课堂
            start() {
                BNService.getDetail(this.id).then((data) => {
                    if (!data.room_id) {
                        this.$Message.error(this.$t('classroom.waitTeacher'))
                        setTimeout(function () {
                            window.opener = null;
                            window.open('', '_self');
                            window.close();
            ...
            initEvent() {
                ...
                this.ticSdk.on(TICSDK.CONSTANT.EVENT.IM.GROUP_SYSTEM_NOTIFYS, imEvent => {
                    if (imEvent.event_type === 5) { // 群被解散
                        ...
                        BNService.getpay(this.id).then((data) => {
                            this.model = true
                        });
