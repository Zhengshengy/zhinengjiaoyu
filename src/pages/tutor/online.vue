<template>
    <div class="demo-split">
        <audio id="audio" src="/client/audio/tishi.wav"></audio>
        <Split v-model="split3">
            <div slot="left" class="demo-split-pane">
                ...
                <div class="col-md-6" style="position: relative">
                    ...
                    <div class="bar">
                        <Icon type="md-color-wand" :title="$t('classroom.background')" :class="{'active':ind === 1}"
                              @click="xuan(1)"/>
                        <Icon type="ios-color-palette" :title="$t('classroom.brush')" :class="{'active':ind === 2}"
                              @click="xuan(2)"/>
                        <Icon type="ios-brush" :title="$t('classroom.Restore')" :class="{'active':ind === 6}"
                              @click="drawline"/>
                        <Icon type="ios-settings-outline" :title="$t('classroom.brush_size')"
                              :class="{'active':ind === 4}" @click="xuan(4)"/>
                        <Icon type="ios-apps-outline" :title="$t('classroom.brush_state')" :class="{'active':ind === 3}"
                              @click="xuan(3)"/>
                        <Icon type="ios-refresh-circle-outline" :title="$t('classroom.rubber')" :class="{'active':ind === 8}"
                              @click="rubber"/>
                        <Icon type="md-trash" :title="$t('classroom.delete')" :class="{'active':ind === 5}"
                              @click="clear"/>
                        <div style="display: inline-block">
                            <label class="custom-file-label" for="file_input" style="cursor: pointer">
                                <Icon type="ios-paper" :title="$t('classroom.Select_upload')" style="margin-right: 0"/>
                            </label>
                            <input id="file_input" type="file"
                                   style="width: 2px;font-size: 1px; opacity: 0;cursor: pointer" @change="uploadFile"
                                   accept="image/*, application/pdf, application/msword, application/vnd.ms-powerpoint, application/vnd.ms-excel">
                        </div>
                    </div>
                    <div class="dis" v-if="index==4">
                        <div class="yuan" @click="setThin(50)" :class="{'actives':indset === 50}">
                            <div style="width: 5px;height: 5px;border-radius: 100%;background: #fff;margin: 17px auto"></div>
                        </div>
                        <div class="yuan" :class="{'actives':indset === 100}" @click="setThin(100)">
                            <div style="width: 10px;height: 10px;border-radius: 100%;background: #fff;margin: 15px auto"></div>
                        </div>
                        <div class="yuan" :class="{'actives':indset === 250}" @click="setThin(250)">
                            <div style="width: 15px;height: 15px;border-radius: 100%;background: #fff;margin: 12px auto"></div>
                        </div>
                        <div class="yuan" :class="{'actives':indset === 300}" @click="setThin(300)">
                            <div style="width: 20px;height: 20px;border-radius: 100%;background: #fff;margin: 10px auto"></div>
                        </div>
                        <div class="yuan" :class="{'actives':indset === 400}" @click="setThin(400)">
                            <div style="width: 25px;height: 25px;border-radius: 100%;background: #fff;margin: 7px auto"></div>
                        </div>
                    </div>
                    <div class="dis" v-else-if="index==1" style="font-size: 16px;padding:0 20px">
                        {{$t('classroom.background')}}&nbsp &nbsp&nbsp&nbsp
                        <ColorPicker v-model="color" @on-change="setBgColor(color)"/>
                    </div>
                    <div class="dis" v-else-if="index==2" style="font-size: 16px;padding:0 20px">
                        {{$t('classroom.brush')}}&nbsp &nbsp&nbsp&nbsp
                        <ColorPicker v-model="color1" @on-change="setColor(color1)"/>
                    </div>
                    <div class="dis dis1" v-else-if="index==3" style="width: 33%;padding:0 20px">
                        <Icon type="md-remove" :title="$t('classroom.straight_line')" :class="{'active':indset === 1}"
                              size="24" @click="setType(1)"/>
                        <Icon type="ios-repeat" :title="$t('classroom.pencil')" :class="{'active':indset === 7}"
                              size="24" @click="setType(7)"/>
                        <Icon type="ios-radio-button-off" :title="$t('classroom.hollow_circle')"
                              :class="{'active':indset === 3}" @click="setType(3)" size="24"/>
                        <Icon type="ios-radio-button-on" :title="$t('classroom.roundness')"
                              :class="{'active':indset === 4}" @click="setType(4)" size="24"/>
                        <Icon type="md-square-outline" :class="{'active':indset === 6}" size="24"
                              :title="$t('classroom.quadrangle')" @click="setType(6)"/>
                        <Icon type="md-square" :title="$t('classroom.quadrangle')" :class="{'active':indset === 5}"
                              size="24" @click="setType(5)" style="margin-right: 0"/>
                    </div>
                </div>
                <div class="di">
                    <Icon type="ios-arrow-dropleft" :title="$t('classroom.previous_page')" @click="prevBoard"/>
                    <span style="margin-right: 10px">{{boardData.page.current}}</span>
                    <Icon type="ios-arrow-dropright" :title="$t('classroom.next_page')" @click="nextBoard"/>
                    <Icon type="md-add-circle" :title="$t('classroom.sketchpad')" @click="addBoard"/>
                    <Icon type="md-trash" style="margin-right:0 " :title="$t('classroom.the_sketchpad')"
                          @click="deleteBoard"/>
                </div>
            </div>
                ...
                <Split v-model="split4" mode="vertical">
                    ...
                    <div slot="bottom" class="demo-split-pane" style="height: 100%">
                        ...
                        <div class="bot" style="line-height: 100%">
                            <input type="text" v-model="imMsg.common.data" size="large"
                                   :placeholder="this.$t('classroom.sendContent')"
                                   @keyup.enter="sendMsg"
                            ...
                            <Button type="info" @click="sendMsg"
                                    style="float: right;height: 70%;margin: 2.5% 15px 0 0;line-height: 100%">
                                {{$t('classroom.send')}}
                            </Button>
                        </div>
                    </div>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            BNService.getValidate().then((data) => {
                if (data.status == false) {
                    this.$Message.error(this.$t('classroom.shenhe'))
                    setTimeout(() => {
                        window.opener = null;
                        window.open('', '_self');
                        window.close();
                    }, 2000)
                } else if (data.subject == false) {
                    this.$Message.error(this.$t('classroom.setCourses'))
                    setTimeout(() => {
                        window.opener = null;
                        window.open('', '_self');
                        window.close();
                    }, 2000)
                } else if (data.is_card == false) {
                    this.$Message.error(this.$t('classroom.authentication'))
                    setTimeout(() => {
                        window.opener = null;
                        window.open('', '_self');
                        window.close();
                    }, 2000)
                } else if (data.is_edu == false) {
                    this.$Message.error(this.$t('classroom.certification'))
                    setTimeout(() => {
                        window.opener = null;
                        window.open('', '_self');
                        window.close();
                    ...
                    this.start()
                    BNService.createQueue().then((data) => {
                        BNUtils.setUserRoomId(data.room_id)
                        this.roomID = data.room_id
                        this.init()
                    }).catch(() => {
                        this.roomID = BNUtils.getUserRoomId()
                        this.init()
                    })
                }
            })
        ...
        methods: {
                    ...
                    msgs.elems.forEach(msg => {
                        ...
                        if (msgs.getFromAccount() === '@TIM#SYSTEM') { // 接收到系统消息
                            ...
                            if (opType === webim.GROUP_TIP_TYPE.JOIN) { // 加群通知
                                ...
                                BNService.getTeacherTextImg().then((data) => {
                                    console.log(data)
                                    if (data.type==1){
                                        this.type=1
                                        ...
                                        this.type=2
                                        let url = BNUtils.domain()
                                        this.videourl = url+'/src/'+data.path
                                    }
                                });
                                //播放音频  ，提示
                                ...
                                audio.play()
                                this.$Modal.success({
                                    title: this.$t('classroom.prompt'),
                                    content: this.$t('classroom.came'),
                                ...
                                this.$Modal.confirm({
                                    title: this.$t('classroom.prompt'),
                                    content: this.$t('classroom.exit'),
                                    okText: this.$t('classroom.yes'),
                                    cancelText: this.$t('classroom.No'),
                                    onOk: () => {
                                        this.ticSdk.destroyClassRoom();
                                        BNUtils.clearUserRoomId()
                                        BNService.createQueue().then((data) => {
                                            BNUtils.setUserRoomId(data.room_id)
                                            this.roomID = data.room_id
                                            this.init();
                                    ...
                                    onCancel: () => {
                                        ...
                                        BNUtils.clearUserRoomId()
                                        BNService.delAskqueue().then((data) => {
                                            this.ticSdk.destroyClassRoom();
                                            setTimeout(() => {
                                                window.opener = null;
                                                window.open('', '_self');
            ...
            setType(num) {
                if (num == 1) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.GRAPH_LINE);
                    this.indset = 1
                } else if (num == 7) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.LINE);
                    this.indset = 7
                } else if (num == 2) {
                    this.ind = 6
                } else if (num == 3) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.GRAPH_OVAL);
                    this.indset = 3
                } else if (num == 4) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.GRAPH_OVAL_SOLID);
                    this.indset = 4
                } else if (num == 5) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.GRAPH_RECT_SOLID);
                    this.indset = 5
                } else if (num == 6) {
                    this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.GRAPH_RECT);
                    this.indset = 6
                }
            },
            ...
            rubber(){
                this.ind = 8
                this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.LINE)
                this.ticSdk.getBoardInstance().setColor('#ffffff')
                this.ticSdk.getBoardInstance().setThin(400);
                this.ticSdk.getBoardInstance().setRadius(400);
            },
            drawline(){
                this.ind = 6
                this.ticSdk.getBoardInstance().setType(BoardSDK.DRAW_TYPE.LINE)
                this.ticSdk.getBoardInstance().setColor(this._color)
                this.setThin(100)
            }
        },
