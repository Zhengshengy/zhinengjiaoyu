            ...
            <div slot="left" class="demo-split-pane">
                <div class="top"
                    ...
                    <Button type="info" style="float: right;height: 70%;margin: 1% 15px 0 0;line-height: 100%"
                            @click="quit">{{$t('classroom.closeClassroom')}}
                    </Button>
                </div>
                    ...
                    <div slot="top" class="demo-split-pane" style="height: 100%;">
                        ...
                        <div style="height: 85%;" v-if="classification==1">
                            <div class="cutindex" v-if="indcut==0">
                                <div :class="me_video">
                                    <video id="local_video" muted="true" autoplay
                                           playsinline></video>
                                </div>
    ...
    export default {
        ...
        async mounted() {
            ...
            BNService.getSign().then((data) => {
                ...
                BNService.getTeaching(this.id).then((data) => {
        ...
        methods: {
            ...
            time() {
                this.timer = setTimeout(() => {
                    BNService.getAskqueueLength(this.id).then((data) => {
                        if (data.status == '1') {
                            this.time()
                        } else if (data.status == '2') {
                            this.$Message.error("时间到了，准备下课")
                            this.$Message.error("5秒后关闭自动关闭课堂")
                            window.clearTimeout(this.timer)
                            let href = window.opener.location.href
                            window.opener.location.replace(href);
                            this.$store.commit("teaching/stop")
                    ...
                    msgs.elems.forEach(msg => {
                        ...
                        if (msgs.getFromAccount() === '@TIM#SYSTEM') { // 接收到系统消息
                            ...
                            if (opType === webim.GROUP_TIP_TYPE.JOIN) { // 加群通知
                                ...
                                audio.play()
                                BNService.getstatus({id: this.id}).then((data) => {
                                    let href = window.opener.location.href
                                    window.opener.location.replace(href);
                                    this.$Modal.success({
                                ...
                                this.$Modal.confirm({
                                    title: this.$t('classroom.prompt'),
                                    content: this.$t('classroom.exitRoom'),
            ...
            quit() {
                this.$Modal.confirm({
                    title: this.$t('classroom.exitClose'),
                    content: this.$t('classroom.quitClose'),
                    okText: this.$t('classroom.determine'),
                    cancelText: this.$t('classroom.return'),
                    onOk: () => {
                        window.clearTimeout(this.timer)
                        BNService.getstatus({id: this.id, type: 1}).then((data) => {
                            let href = window.opener.location.href
                            window.opener.location.replace(href);
                            this.ticSdk.destroyClassRoom();
                            setTimeout(() => {
