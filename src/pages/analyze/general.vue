        ...
        <div class="exam-main">
            ...
            <div v-else-if="paperInfo.exercise" class="exam-main-container">
                ...
                <div class="exam-main-left">
                    <header>
                        <div>
                            <span v-if="false && paperInfo.ability" >{{$t('analyze.cur_level')}}：{{level[`${paperInfo.ability.level}`]}}</span>
                            <span >{{$t('analyze.target')}}：{{level[target]}}</span>
                        </div>
...
export default {
    ...
    async mounted() {
        this.getGeneralTrainingExercise({
    ...
    methods: {
        ...
        ...mapActions({
            getGeneralTrainingExercise: Constants.PAPER.GET_GENERAL_TRAINING_EXERCISE,
            submitGeneralTrainingExercise: Constants.PAPER.SUBMIT_GENERAL_TRAINING_EXERCISE,
            nextGeneralTrainingExercise: Constants.PAPER.NEXT_GENERAL_TRAINING_EXERCISE,
            finishGeneralTrainingExercise: Constants.PAPER.FINISH_GENERAL_TRAINING_EXERCISE,
        ...
        handleSubmit() {
            const self = this;
            this.changeFeedbackState(false);
            self.submitGeneralTrainingExercise({
        ...
        handleNext() {
            ...
            if (self.paperInfo.complete_at || self.paperInfo.active_number === self.paperInfo.total) {
                this.finishGeneralTrainingExercise({
                    id: self.paperInfo.id
                });
                BNRouteHelper.analyzeResult(this.$router);
            } else {
                this.nextGeneralTrainingExercise({
                    id: self.paperInfo.id
                }).catch((error) => {
                    this.$Message.error(error.message);
                    this.error = error;
